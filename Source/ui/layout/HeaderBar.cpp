...
void HeaderBar::resized()
{
    ...
    const int headerPadX = m.pad;
    const int controlH = m.headerButtonH;
    const int comboW = m.headerDbRangeBoxW;
    const int smallBtnW = m.headerResetButtonSize;
    const int minButtonW = m.buttonSmallW;
    const int minTitleW = 60;
    int headerGap = m.gap;
    int buttonW = m.headerButtonW;
    ...
    const int leftZonePrefW = comboW * 3 + headerGap * 2;
    const int rightZoneFixedW = comboW * 2 + headerGap + smallBtnW + headerGap + headerGap * 2;
    int availableForButtons = area.getWidth() - (leftZonePrefW + minTitleW + rightZoneFixedW);
    if (availableForButtons < m.headerButtonW * 3)
        buttonW = juce::jlimit (minButtonW, m.headerButtonW, availableForButtons / 3);

    if (buttonW == minButtonW)
    {
        headerGap = m.gapSmall;
        const int leftZoneCompactW = comboW * 3 + headerGap * 2;
        const int rightZoneCompactFixedW = comboW * 2 + headerGap + smallBtnW + headerGap + headerGap * 2;
        availableForButtons = area.getWidth() - (leftZoneCompactW + minTitleW + rightZoneCompactFixedW);
        buttonW = juce::jlimit (minButtonW, m.headerButtonW, availableForButtons / 3);
    }
    ...
    const int rightZoneWidth = comboW * 2  // dB Range + Peak Range combos
                              ...
                              + headerGap
                              + buttonW * 3  // Preset/Save/Menu
                              + headerGap * 2;
    auto rightZone = area.removeFromRight (rightZoneWidth);
    
    // Right zone layout (right-to-left)
    auto buttonArea = rightZone.removeFromRight (buttonW);
    menuButton.setBounds (buttonArea.getX(), buttonTop, buttonW, controlH);
    rightZone.removeFromRight (headerGap);
    
    buttonArea = rightZone.removeFromRight (buttonW);
    saveButton.setBounds (buttonArea.getX(), buttonTop, buttonW, controlH);
    rightZone.removeFromRight (headerGap);
    
    buttonArea = rightZone.removeFromRight (buttonW);
    presetButton.setBounds (buttonArea.getX(), buttonTop, buttonW, controlH);
    rightZone.removeFromRight (headerGap);
