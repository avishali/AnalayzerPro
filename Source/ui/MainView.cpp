      ...
      inputMeters_ (ui_, p, MeterGroupComponent::GroupType::Input)
{
    ...
    rail_.setGainChangedCallback ([this] (float gain)
    {
        audioProcessor.getParameters().setGain (gain);
    });
    rail_.setGainValue (audioProcessor.getParameters().getGain());
...
void MainView::resized()
{
    ...
    // 3) Calculate responsive controls height (fit rail content at minimum size)
    const int minAnalyzerH = 90;
    int controlsH = rail_.getPreferredHeight();
    const int maxControlsH = juce::jmax (0, bounds.getHeight() - minAnalyzerH);
    controlsH = juce::jmin (controlsH, maxControlsH);
